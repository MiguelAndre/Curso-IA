...
> **Todo el código nuevo es OO — incluidos los programas de diálogo/RF** (decisión del usuario 2026-09-17). La lógica de negocio vive en **clases y métodos**, nunca en FORMS/PERFORM. Para programas de diálogo/RF con dynpros: usa un **controlador OO** (clase local `lcl_ctrl_<...>` o global `ZCL_<...>`) donde vive toda la lógica; los módulos **PBO/PAI son finos** y solo delegan (`go_ctrl->pbo_9000( )`, `go_ctrl->pai_9000( )`); solo permanecen como globales los **campos de dynpro y `ok_code`** (requisito de binding de pantalla). Esto **supersede el patrón procedural de FORMS `ZWMI_*_F01`** del estándar RF para **nuevos** desarrollos — pero **conserva** todas las demás convenciones RF (dynpros `9xxx` 38×17, pila `ti_screen`, mensajes 9990, paginación manual, caveats de INITIALIZATION, `L_TO_*`, menú RF; ver memoria `estandar-rf-wm`). Los programas productivos existentes (`ZWM064A`) se mantienen procedurales; no los reescribas salvo petición explícita.

> **Estilo moderno (ABAP 7.40+)**: declara **inline** por defecto — `DATA(lo_x) = NEW cl_...( )`, `DATA(lt) = VALUE #( ... )`, `DATA(ls) = ...`, `FIELD-SYMBOL(<fs>)`. Evita `DATA:`+`CREATE OBJECT` y variables globales innecesarias (usa el menor alcance). Usa **métodos estáticos** (`CLASS-METHODS`, `cl_...=>m( )`) cuando necesites algo de la clase **antes de instanciarla** (value helps, utilidades sin estado, `AT SELECTION-SCREEN ON VALUE-REQUEST`). **Excepción única**: solo si el usuario indica que el sistema es **ABAP < 7.40**, vuelve al patrón clásico (`DATA:` + `CREATE OBJECT`).
