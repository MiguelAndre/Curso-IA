...
1. **Parsear los argumentos**. Espera 1 ó 2:
   - `<ruta-td>` (obligatorio): ruta a un archivo `.md` con un TD aprobado (típicamente `outputs/<req-id>/td.md` o `td-vN.md`).
   - `<req-id>` (opcional): identificador del requerimiento. Habilita persistencia en `outputs/<req-id>/codigo[-vN].abap`.

2. **Validar argumentos**:
   - Si no se pasó ningún argumento:
     > "Uso: `/generar-abap <ruta-td> [req-id]`. Ejemplo: `/generar-abap outputs/REQ-2026-042/td.md REQ-2026-042`"
     y termina.
...
3. **Detectar regeneración**:
   - Si se pasó `<req-id>`, verifica si ya existen archivos `codigo*.abap` en `outputs/<req-id>/`.
...
5. **Invocar al sub-agente `td-a-codigo`** usando la tool `Agent`:
   ...
   - `prompt`: incluir:
     ...
     - "Si se pasó `<req-id>`, persiste el código con `Write` en `outputs/<req-id>/codigo[-vN].abap` siguiendo el versionado de tu §8."
     - "Si el código depende de textos u objetos a crear a mano en SAP (símbolos de texto, textos de selección, mensajes SE91, lock objects, objetos de autorización, transacciones, roles), persiste también `outputs/<req-id>/textos-y-objetos.md` (§1 / §8)."
...
6. **Procesar el resultado**:
   ...
   - **Si el sub-agente persistió el archivo**: confirma:
     > "Código persistido en `outputs/<req-id>/codigo[-vN].abap`."
...
```
/generar-abap outputs/REQ-2026-042/td.md REQ-2026-042
```

Comportamiento esperado:
1. Validar archivo TD.
2. Crear `outputs/REQ-2026-042/` si no existe.
