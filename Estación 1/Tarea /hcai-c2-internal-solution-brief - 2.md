...
**Visión:**
Un requerimiento de complejidad media que hoy toma ~17 días hábiles de ciclo completo pasa a resolverse en **4–5 días**. El cuello de botella deja de ser el tiempo de desarrollo y se convierte en la disponibilidad del consultor funcional y el área de procesos para hacer pruebas — un recurso coordinable, no un recurso técnico escaso. La cola de 3 meses se reduce progresivamente a medida que el agente libera capacidad de desarrollo.
...
- El desarrollador ABAP no haya reescrito más del 30% del código generado en ninguno de esos requerimientos
...
**Herramientas — Sin restricción conocida:**
No existe política formal que restrinja herramientas externas. El único IDE externo en uso actual es Eclipse, que es el IDE estándar de ABAP. Cualquier solución que opere como extensión de Eclipse puede implementarse sin requerir aprobación de un nuevo entorno de desarrollo.
...
```
[Documento Funcional]          [Agente 1]              [Agente 2]           [Desarrollador ABAP]
(Formato estándar empresa) --> FD → Documento  -->  Documento Técnico --> Revisa, prueba y
(Word / PDF)                   Técnico               → Código ABAP         aprueba el .abap
                               (Claude Code)          (.abap file)          │
                                    │                 (Claude Code)          ▼
                                    │                                  Importa manualmente
                                    ▼                                  a SAP Development
                              [SAP Dev - Solo lectura]
...
Claude Code opera con el documento funcional estándar como input. El desarrollador orquesta los dos pasos (FD→TD y TD→código) usando Claude Code en su flujo de trabajo actual junto a Eclipse. Sin conexión directa a SAP — el agente trabaja solo con los documentos. Objetivo: completar 4-5 requerimientos exitosos (criterio de la Sección 5).

**Fase 2 — Escala (si el piloto es exitoso):**
Se añade un MCP Server de solo lectura para que el agente consulte estructuras reales del sistema SAP Development. Se evalúa Anthropic API para automatizar el pipeline completo y reducir la intervención manual del desarrollador en la orquestación. El desarrollador pasa a rol puramente de auditor y aprobador.
...
| 2 | El código generado tiene errores de compilación o lógica incorrecta | Media | Alto | El desarrollador ABAP ejecuta syntax check y pruebas unitarias antes de importar cualquier `.abap`. Esta compuerta ya existe hoy — no es nueva. | Desarrollador ABAP |
